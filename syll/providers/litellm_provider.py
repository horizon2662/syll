...
class LiteLLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        self.is_vllm = (
            ...
            and not self.is_anthropic_endpoint  # Anthropic endpoints are NOT vLLM
        )

        # Configure LiteLLM based on provider
        if api_key:
            if self.is_anthropic_endpoint:
                # Anthropic-compatible endpoint — needs Anthropic SDK env var
    ...
    def _prepare_model(self, model: str | None) -> str:
        ...
        # Anthropic-compatible endpoint: force anthropic/ prefix
        # so LiteLLM sends Anthropic Messages API format
        if self.is_anthropic_endpoint:
            if not model.startswith("anthropic/"):
                model = f"anthropic/{model}"
            return model

        # vLLM/custom endpoint mode: only add hosted_vllm/ prefix.
        # Skip litellm provider prefixes (zai/, moonshot/, gemini/ etc.)
        # because the model name is sent verbatim to the custom api_base.
        if self.is_vllm:
    ...
    async def chat(
        ...
        # Pass api_base directly for custom endpoints (vLLM, Anthropic, etc.)
        if self.api_base and not self.is_openrouter:
