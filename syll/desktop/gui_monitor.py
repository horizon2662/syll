...
# ── Orphan detection ────────────────────────────────────────────────────────
# The monitor subprocess needs to know when its parent (the Syll agent) has
# exited so it can auto-hide and clean up.  On Windows there is no PR_SET_PDEATHSIG
# equivalent, so we pass ``--parent-pid`` and poll it.
...
def _is_parent_alive() -> bool:
    ...
    if _parent_pid is None:
        return True  # no tracking → assume alive (standalone mode)
    try:
        if sys.platform == "win32":
            # OpenProcess succeeds even for exited processes; use wait to check.
            import ctypes
...
def launch_gui_monitor() -> None:
    ...
    Uses ``python -m syll.desktop.gui_monitor --serve --parent-pid <PID>``
    so Qt runs in its own main thread.  The child polls the parent PID and
    auto-exits when the parent dies.
    ...
    try:
        cmd = [
            sys.executable, "-m", "syll.desktop.gui_monitor",
            "--serve",
...
def _run_qt_app(serve: bool = False) -> None:
    ...
    class _MonitorWindow(QWidget):
        ...
        def _tick(self):
            # ── Orphan detection: if parent died, hide & quit immediately ──
            if not _is_parent_alive():
                logger.info("GUI monitor: parent process exited, shutting down")
                self.hide()
                clear_gui_state()
                QApplication.quit()
                return
            ...
            try:
                if not _STATE_FILE.exists():
                    self._idle_ticks += 1
                    # Auto-exit if idle for too long (serve mode)
                    if serve and self._idle_ticks > MAX_IDLE_TICKS:
                        QApplication.quit()
                        return
                    return
                ...
                # If the timestamp is older than 30 s, treat as stale → hide
                ts_str = new.get("ts", "")
                if ts_str:
                    try:
...
if __name__ == "__main__":
    ...
    # Parse --parent-pid for orphan detection
    for i, arg in enumerate(sys.argv):
        if arg == "--parent-pid" and i + 1 < len(sys.argv):
            try:
                _parent_pid = int(sys.argv[i + 1])
