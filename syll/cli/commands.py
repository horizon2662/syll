...
# ============================================================================
# Skill Optimizer Commands (SkillOpt integration)
# ============================================================================


skillopt_app = typer.Typer(
    help="Optimize skills using the SkillOpt training loop.",
    no_args_is_help=True,
)
...
def skillopt_train(
    ...
    tasks_file: str = typer.Option(
        None, "--tasks", "-t",
        help="Path to tasks JSON file (default: workspace/skill_optimizer/<name>/tasks.json)",
    ),
    eval_file: str = typer.Option(
        None, "--eval", "-e",
        help="Path to eval tasks JSON file (default: auto-split from training tasks)",
    ),
    epochs: int = typer.Option(None, "--epochs", help="Number of training epochs"),
    batch_size: int = typer.Option(None, "--batch-size", help="Tasks per rollout batch"),
    budget: int = typer.Option(None, "--budget", "-b", help="Max edits per step"),
    gate: str = typer.Option(None, "--gate", help="Gate metric: hard, soft, mixed"),
    output: str = typer.Option(None, "--output", "-o", help="Output directory"),
    apply: bool = typer.Option(
        False, "--apply",
        help="Apply the best skill back to the workspace after training",
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v"),
):
    """Optimize a skill through iterative training (SkillOpt ReflACT loop).

    This command runs the 6-stage training loop inspired by Microsoft SkillOpt:
    rollout → reflect → aggregate → select → update → evaluate.

    Tasks are loaded from a JSON file with the format:
    \b
        [
          {"id": "1", "task": "Find report.docx", "expected": "/path/to/report.docx"},
          ...
        ]

    If no tasks file is specified, it looks for:
      ~/.syll/workspace/skill_optimizer/<skill_name>/tasks.json

    Example:
    \b
        syll skillopt train file-retrieval --tasks my_tasks.json --epochs 3
    """
    from syll.agent.loop import AgentLoop
    from syll.bus.queue import MessageBus
    from syll.config.loader import load_config
    ...
    config = load_config()
    opt_config = config.skill_optimizer
    workspace = config.workspace_path

    # Resolve tasks file
    if not tasks_file:
        default_path = workspace / "skill_optimizer" / skill_name / "tasks.json"
        if not default_path.exists():
            console.print(
                f"[red]No tasks file found.[/red]\n"
                f"  Expected: {default_path}\n"
                f"  Or specify: --tasks path/to/tasks.json"
            )
            raise typer.Exit(1)
        tasks_file = str(default_path)
    ...
    console.print(f"[green]✓[/green] Loaded {len(tasks)} tasks from {tasks_file}")

    # Load eval tasks
    eval_tasks = []
    if eval_file:
        try:
            eval_tasks = load_tasks(eval_file)
            console.print(f"[green]✓[/green] Loaded {len(eval_tasks)} eval tasks from {eval_file}")
        except (FileNotFoundError, ValueError) as e:
            console.print(f"[red]Error loading eval tasks:[/red] {e}")
            raise typer.Exit(1)

    # Build LLM provider and agent loop
    chat = config.models.chat
    model = chat.model
    api_key = chat.api_key
    api_base = chat.api_base
    ...
    if not api_key and not is_bedrock:
        console.print("[red]Error: No API key configured.[/red]")
        console.print("  Set models.chat.api_key in ~/.syll/config.json")
        raise typer.Exit(1)
    ...
    optimizer = SkillOptimizer(
        ...
        agent_loop=agent_loop,
        workspace=workspace,
        num_epochs=epochs or opt_config.num_epochs,
        batch_size=batch_size or opt_config.batch_size,
        edit_budget=budget or opt_config.edit_budget,
        minibatch_size=opt_config.minibatch_size,
        gate_metric=gate or opt_config.gate_metric,
        failure_only=opt_config.failure_only,
        out_root=output or "",
        seed=opt_config.seed,
        max_concurrent_rollouts=opt_config.max_concurrent_rollouts,
    )

    # Run training
    summary = asyncio.run(optimizer.train())

    # Apply best skill to workspace if requested
    if apply and summary.get("best_score", 0) > 0:
        best_skill_path = os.path.join(optimizer.out_root, "best_skill.md")
        if os.path.exists(best_skill_path):
            with open(best_skill_path, encoding="utf-8") as f:
                best_content = f.read()
            optimizer.save_skill(best_content)
            console.print(f"[green]✓[/green] Applied best skill to {optimizer._skill_path()}")

    return summary


@skillopt_app.command("list-tasks")
def skillopt_list_tasks(
    skill_name: str = typer.Argument(None, help="Skill name to list tasks for"),
):
    """List available task files for skill optimization."""
    from syll.config.loader import load_config

    config = load_config()
    workspace = config.workspace_path
    opt_dir = workspace / "skill_optimizer"

    if not opt_dir.exists():
        console.print("[dim]No skill_optimizer directory found.[/dim]")
        console.print(f"  Create tasks at: {opt_dir}/<skill_name>/tasks.json")
        return

    table = Table(title="Skill Optimizer Tasks")
    table.add_column("Skill", style="cyan")
    table.add_column("Tasks File", style="green")
    table.add_column("Count", style="yellow")

    for skill_dir in sorted(opt_dir.iterdir()):
        ...
        if tasks_file.exists():
            try:
                ...
                table.add_row(name, str(tasks_file), str(len(tasks)))
            except Exception as e:
                table.add_row(name, str(tasks_file), f"[red]error: {e}[/red]")
        else:
            table.add_row(name, "[dim]no tasks.json[/dim]", "-")

    console.print(table)


@skillopt_app.command("create-tasks-template")
def skillopt_create_template(
    skill_name: str = typer.Argument(..., help="Skill name"),
    n_examples: int = typer.Option(5, "--count", "-n", help="Number of example slots"),
):
    """Create a tasks.json template for a skill."""
    from syll.config.loader import load_config

    config = load_config()
    workspace = config.workspace_path
    tasks_dir = workspace / "skill_optimizer" / skill_name
    tasks_dir.mkdir(parents=True, exist_ok=True)

    tasks_file = tasks_dir / "tasks.json"
    if tasks_file.exists():
        if not typer.confirm(f"Overwrite {tasks_file}?"):
            raise typer.Exit()

    template = [
        {
            "id": str(i + 1),
            "task": f"Example task {i + 1} for the {skill_name} skill",
    ...
    console.print(f"[green]✓[/green] Created tasks template at {tasks_file}")
    console.print(f"  Edit it with your actual tasks, then run:")
    console.print(f"  [cyan]syll skillopt train {skill_name}[/cyan]")


@skillopt_app.command("eval-skill")
def skillopt_eval(
    skill_name: str = typer.Argument(..., help="Skill name to evaluate"),
    tasks_file: str = typer.Option(
        None, "--tasks", "-t",
        help="Path to tasks JSON file",
    ),
):
    """Evaluate a skill on a set of tasks (no training)."""
    from syll.agent.loop import AgentLoop
    from syll.bus.queue import MessageBus
    from syll.config.loader import load_config
    from syll.providers.litellm_provider import LiteLLMProvider
    from syll.skill_optimizer.rollout import load_tasks, rollout_batch
    from syll.skill_optimizer.types import compute_score
    ...
    if not tasks_file:
        default_path = workspace / "skill_optimizer" / skill_name / "tasks.json"
        if not default_path.exists():
            console.print(f"[red]No tasks file found for '{skill_name}'[/red]")
            raise typer.Exit(1)
        tasks_file = str(default_path)
    ...
    # Run evaluation
    console.print(f"\nEvaluating skill '{skill_name}' on {len(tasks)} tasks...\n")
    results = asyncio.run(rollout_batch(agent_loop, tasks, max_concurrent=2))
    hard, soft = compute_score(results)

    console.print(f"[bold]Results:[/bold]")
    console.print(f"  Hard accuracy: {hard:.2%} ({int(hard * len(results))}/{len(results)})")
    console.print(f"  Soft score:    {soft:.2%}")
    ...
    table.add_column("Fail Reason", width=30)
    for r in results:
        status = "[green]✓[/green]" if r.hard else "[red]✗[/red]"
        task_preview = r.task[:48] + ".." if len(r.task) > 48 else r.task
        reason = r.fail_reason[:28] if r.fail_reason else ""
        table.add_row(r.id, status, task_preview, reason)
    console.print(table)
