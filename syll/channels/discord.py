...
from syll.config.schema import DiscordConfig
...
class DiscordChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            if has_media:
                ...
                files = {}
                attachments = []
                for i, path in enumerate(msg.media):
                    p = Path(path)
                    if not p.is_file():
                        continue
                    files[f"files[{i}]"] = (p.name, p.read_bytes())
                    attachments.append({"id": i, "filename": p.name})

                payload_json: dict[str, Any] = {"content": msg.content}
                if attachments:
                ...
                for attempt in range(3):
                    try:
                        ...
                        if attempt == 2:
                            logger.error(f"Error sending Discord message with media: {e}")
                        else:
    ...
    async def _handle_message_create(self, payload: dict[str, Any]) -> None:
        ...
        media_dir = Path.home() / ".syll" / "media"
