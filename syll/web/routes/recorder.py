...
def _transcode_with_imageio(source_path: Path, output_path: Path) -> Path:
    """Transcode video to H.264 using imageio (pure Python, no system ffmpeg).

    This is the primary fallback when system ffmpeg is not installed.
    ...
    try:
        for frame in reader:
            # imageio yields RGB; H.264 encoder handles it fine
...
def _transcode_with_cv2(source_path: Path, output_path: Path) -> Path:
    """Last-resort transcoding using OpenCV (only if imageio is unavailable)."""
    try:
        import cv2
    except ImportError:
    ...
    if writer is None:
        cap.release()
        raise RuntimeError("Failed to create H.264 VideoWriter")
...
def _ensure_browser_video(video_path: Path) -> Path:
    ...
    with lock:
        if browser_path.exists() and browser_path.stat().st_mtime >= video_path.stat().st_mtime:
            return browser_path
        # 1. system ffmpeg (fastest)
        try:
            return _transcode_video_for_browser(video_path, browser_path)
        except RuntimeError:
            pass
        # 2. imageio with bundled ffmpeg (pure Python, no system dep)
        try:
            return _transcode_with_imageio(video_path, browser_path)
        except Exception:
            pass
        # 3. cv2 (last resort)
        return _transcode_with_cv2(video_path, browser_path)
...
def _read_video_frame_bytes(video_path: Path, timestamp_ms: int) -> bytes:
    from syll.recorder.screen_capture import read_frame_at_ms, encode_frame_jpeg

    frame = read_frame_at_ms(str(video_path), timestamp_ms)
