"""API routes for recorded workflow CRUD, import, execution, and scheduling."""
...
router = APIRouter(prefix="/recorded-workflows", tags=["recorded-workflows"])
...
def _route(path: str, method: str, **kwargs):
    """Register a canonical public route plus a hidden legacy alias."""
...
class AlohaActionModel(BaseModel):
    type: str
...
class AlohaTraceModel(BaseModel):
    observation: str = ""
    think: str = ""
    action: str = ""
    expectation: str = ""
...
class CreateAlohaSkillRequest(BaseModel):
    name: str
    description: str = ""
    app_context: str = ""
    source: str = "manual"
...
class AlohaStepCreate(BaseModel):
    action: AlohaActionModel
    screenshot_b64: str
    screenshot_mime: str = "image/jpeg"
    trace: AlohaTraceModel | None = None


class UpdateStepRequest(BaseModel):
    action: AlohaActionModel | None = None
    trace: AlohaTraceModel | None = None
...
def _get_store(request: Request):
    return request.app.state.aloha_skill_store
...
def _encode_screenshots(paths: list[str]) -> list[dict]:
    ...
    for p in paths:
        path = Path(p)
        if path.exists():
            data = b64module.b64encode(path.read_bytes()).decode()
            mime = mimetypes.guess_type(str(path))[0] or "image/png"
            result.append({"data": data, "mime": mime, "filename": path.name})
    return result


def _auto_fill_description(skill: AlohaSkill) -> str:
...
def _build_icl_rich_message(name: str, instruction: str) -> str:
    """Build a deterministic prompt for rich recorded-workflow replay."""
    return (
...
@_route("", "POST")
async def create_aloha_skill(body: CreateAlohaSkillRequest, request: Request):
    """Create an empty recorded workflow."""
...
@_route("", "GET")
async def list_aloha_skills(request: Request):
...
@_route("/{name}", "GET")
async def get_aloha_skill(name: str, request: Request):
    """Get a recorded workflow with all steps."""
    store = _get_store(request)
    skill = store.load_skill(name)
    if not skill:
        raise HTTPException(404, f"Recorded workflow '{name}' not found")
    return {
...
@_route("/{name}", "DELETE")
async def delete_aloha_skill(name: str, request: Request):
...
async def import_recording(name: str, body: ImportRecordingRequest, request: Request):
    ...
    # Resolve trace endpoint via purpose-based config
    trace_model = "gpt-4o"
    api_key = None
    api_base = None
    if config:
    ...
    importer = AlohaImporter(
        store=store, trace_model=trace_model, api_key=api_key, api_base=api_base
    )

    try:
        skill = await importer.import_recording(
...
@_route("/{name}/generate-trace", "POST")
async def generate_trace(name: str, request: Request):
    ...
    try:
        skill = await importer.generate_trace(name)
        return {
            "name": name,
...
@_route("/{name}/steps", "POST")
async def add_step(name: str, body: AlohaStepCreate, request: Request):
    ...
    step = AlohaStep(index=idx, screenshot=filename, action=action, trace=trace)
...
@_route("/{name}/keyframes/{filename}", "GET")
...
@_route("/{name}/execute", "POST")
async def execute_aloha_skill(name: str, request: Request, body: ExecuteRequest | None = None):
    """Execute a recorded workflow via the agent loop."""
    store = _get_store(request)
    skill = store.load_skill(name)
    if not skill:
        raise HTTPException(404, f"Recorded workflow '{name}' not found")
    ...
    if mode == "planner":
        ...
        try:
            agent_result = await agent_loop.process_direct(
                content=message,
                session_key=f"aloha-skill:{name}",
            )
            return {
                "status": "executed",
                "mode": mode,
            ...
            logger.error(f"Aloha skill execution failed: {e}")
            raise HTTPException(500, str(e))


@_route("/{name}/schedule", "POST")
async def schedule_aloha_skill(name: str, body: ScheduleRequest, request: Request):
    """Schedule a recorded workflow for cron execution."""
    store = _get_store(request)
    skill = store.load_skill(name)
    if not skill:
        raise HTTPException(404, f"Recorded workflow '{name}' not found")
    ...
    job = cron_service.add_job(
        name=f"aloha-skill:{name}",
        schedule=schedule,
        message=message,
        metadata={
            "action_type": "aloha_skill",
            "skill_name": name,
            "aloha_mode": body.mode,
            "aloha_actor_mode": actor_mode,
        },
    )
